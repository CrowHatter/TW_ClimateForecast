...
本專題旨在使用深度學習技術來進行天氣資料的預測。透過資料前處理、模型訓練與評估，建立一個能夠預測未來氣象變化的系統，期望提升氣象預測的準確度與應用價值。
...
- 將非數值欄位轉換為數值型態
- 進行 Z-score 正規化
- 轉換為適合 Transformer 的時間序列格式

### 2. 模型訓練
- 使用 PyTorch 架構並以 CUDA 加速訓練 Transformer 模型
- 輸入為過去數天的氣象資料
...
- [ ] 使用 PyTorch 架設初步 Transformer 模型
- [ ] 優化模型架構與參數調整
- [ ] 預測結果評估與視覺化
- [ ] GUI整合與預測輸出展示
